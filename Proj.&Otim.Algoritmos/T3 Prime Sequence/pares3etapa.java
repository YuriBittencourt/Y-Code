public class pares3etapa {
	...
	static void montaSequencia(int sequence[], int n) {
		for(int i=1;i<=sequence.length;i++) {
			...
			if(contains(i,sequence)
						||(n!=0 &&!isPrime(sequence[n-1]+i))
						||(n==sequence.length-1 && !isPrime(sequence[0]+i))
						||(n>=(sequence.length)/2 && !isPrime(i + sequence[(n-sequence.length/2)]))) 
						continue;
			sequence[n]=i;
